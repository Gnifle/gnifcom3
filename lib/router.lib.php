...
class Router {
	...
	public function run() {
			...
			foreach ( $routes as $route => $callback ) {
				...
				if ( preg_match( $route, $this->url_clean, $matches ) ) {
					...
					return array( 'callback'       => $callback,
					              'params'         => $params,
					              'route'          => $route,
					              'original_route' => $this->routes_original[ $priority ][ $route ]
					);
				}
			}
		}
		...
		if ( ! $matched_route && $this->default_route !== null ) {
			$this->params   = array( $this->url_clean );
			$this->callback = $this->default_route;

			return array( 'params'         => $this->url_clean,
	...
	public function dispatch() {
		...
		if( is_callable( $this->callback ) ) {
			...
			call_user_func_array( array( 'Router', $this->callback ), $this->params );
		}
